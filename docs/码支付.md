...
商户ID	pid	是	Int	1001	
支付方式	type	否	String	alipay	支付方式
...
支付跳转url	payurl	String	https://pay.akwl.top/pay/wxpay/202010903/	如果返回该字段，则直接跳转到该url支付
...
jump	仅返回支付跳转url
[API]查询商户信息
URL地址：https://域名/api.php?act=query&pid={商户ID}&key={商户密钥}
...
[API]查询单个订单
URL地址：https://域名/api.php?act=order&pid={商户ID}&key={商户密钥}&out_trade_no={商户订单号}
...
默认留空
[API]批量查询订单
URL地址：https://域名/api.php?act=orders&pid={商户ID}&key={商户密钥}
